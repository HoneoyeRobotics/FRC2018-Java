...
public class OI {
  ...
  public static final int DRIVER_JOYSTICK_PORT = 0;
	...
	public OI() {	
		...
		//set driver joystick
		new JoystickButton(driverJoystick, REVERSE_DRIVE_BUTTON).whenPressed(new frc.robot.commands.ReverseDriveDirection());
		
		//set copilot joystick
		
			
		//set button X to be chew
		new JoystickButton(coPilotJoystick, ARM_CHEW_BUTTON).whileHeld(new frc.robot.commands.Chew());
		//set button Y to arms spit position
		new JoystickButton(coPilotJoystick,ARM_SPIT_POS_BUTTON).whenPressed(new frc.robot.commands.ArmsToSpit());
		//set button A to be arms down
		new JoystickButton(coPilotJoystick,ARM_LOWER_BUTTON).whenPressed(new frc.robot.commands.ArmsDown());
		//set button B to be spit
		new JoystickButton(coPilotJoystick, ARM_SPIT_BUTTON).whileHeld(new frc.robot.commands.Spit());
		
		//set button LB to be release climb latch
		new JoystickButton(coPilotJoystick, CLIMB_LATCH_RELEASE_BUTTON).whenPressed(new frc.robot.commands.ReleaseClimbHook());
		//set button RB to be arm home reset
		new JoystickButton(coPilotJoystick, ARM_RESET_POS_BUTTON).whenPressed(new frc.robot.commands.ArmsToTop());
